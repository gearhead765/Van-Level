...
ESP32 Based digital level to display leveling information for a camper van. This originally started as an ESPHome application however, the requirement to have Home Assistant running on your phone or tablet in order to see the leveling information was cumbersome.  It was decided that displaying the information on a simple web page, instead of a Home Assistant Lovelace overview, would be much easier to use.
...
1. Your browser should now be displaying the app setup page with the `status` tab selected:
![](Images/settings_status.png)
